...
class RuanganService {
  ...
  void updateRuangan(String kodeRuanganLama, Ruangan ruanganBaru) {
    final index = AppData.daftarRuangan.indexWhere((r) => r.kodeRuangan == kodeRuanganLama);
    if (index != -1) {
  ...
  String? hapusRuangan(String kodeRuangan) {
    final sedangDigunakan = AppData.daftarKelas.any((k) => k.kodeRuangan == kodeRuangan);
    if (sedangDigunakan) {
  ...
  bool isBentrokRuangan(String kodeRuangan, String hari, String jamMulai, String jamSelesai, {String? excludeKelasId}) {
  ...
  List<KelasKuliah> kelasPerRuangan(String kodeRuangan) {
    return AppData.daftarKelas.where((k) => k.kodeRuangan == kodeRuangan).toList();
  }
}
